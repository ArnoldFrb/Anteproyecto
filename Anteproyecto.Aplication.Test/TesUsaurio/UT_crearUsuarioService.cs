...
namespace Anteproyecto.Aplication.Test.TesUsaurio
{
    ...
    public class UT_crearUsuarioService
    {
        ...
        public void crearUsuarioTest()
        {
            ...
            //Act
            var _user = new crearUsuarioRequest { Nombres = user.Nombres, Apellidos = user.Apellidos, NumeroIdentificacion = "0123456710", Correo = user.NumeroIdentificacion, Contraseña = user.Contraseña};
            var response = _crearusuarioService.CrearCuentaBancaria(_user);

            //Assert
            Assert.AreEqual("Se ha creado un nuevo usuario con la identificacion: 0123456710", response.Mensaje);
